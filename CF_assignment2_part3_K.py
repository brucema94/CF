...
for i in range(IterationIndex): 
    Analytical_value = Asian_analytical(S0, r, sigma, T, N, Strikegrid[i])
    Arit, Geo = Asian_MC(S0, r, sigma, T, N, M, Strikegrid[i])
    CV = Control_variate(S0, r, sigma, T, N, M, Strikegrid[i])
...
plotP.plot(Strikegrid, AritMC[:,0],label = "Arit_mean")
plotP.plot(Strikegrid, CV_estimate[:,0],label = "Control Variate")
plotP.set_xlabel("Number of path")
plotP.set_ylabel("Option price")
plotP.legend(loc="upper right")
plotQ.plot(Strikegrid, CV_estimate[:,1],label = "Control_Variate_error")
